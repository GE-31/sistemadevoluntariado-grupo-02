...
function abrirModalEditar(id) {
    ...
    fetch(`voluntarios?action=obtener&id=${id}`)
        ...
        .then(voluntario => {
            ...
            document.getElementById('cargo').value = voluntario.cargo || 'Voluntario';
...
function guardarVoluntario(event) {
    ...
    const cargo = document.getElementById("cargo") ? document.getElementById("cargo").value : 'Voluntario';
    ...
    fetch('voluntarios?action=listar')
        ...
        .then(voluntarios => {
            ...
            if (voluntarios && voluntarios.length > 0) {
                ...
                const fila = document.createElement('tr');
                fila.innerHTML = '<td colspan="9" class="text-center">No hay voluntarios registrados</td>';
                tbody.appendChild(fila);
            }
