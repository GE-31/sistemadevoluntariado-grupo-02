...
function abrirModalCrear() {
    ...
    // Recargar lista actualizada de voluntarios disponibles
    const sel = document.getElementById('voluntarioId');
    sel.innerHTML = '<option value="">Cargando...</option>';
    fetch('usuarios?action=voluntarios')
        .then(r => r.json())
        .then(lista => {
            sel.innerHTML = '<option value="">Seleccione un voluntario</option>';
            if (lista.length === 0) {
                sel.innerHTML += '<option value="" disabled>No hay voluntarios disponibles</option>';
            } else {
                lista.forEach(v => {
        ...
        .catch(() => {
            sel.innerHTML = '<option value="">Error al cargar voluntarios</option>';
        });
...
function abrirModalEditar(id) {
    ...
    fetch(`usuarios?action=obtener&id=${id}`)
        ...
        .then(usuario => {
            ...
            if (usuario.rolSistema) document.getElementById('rolSistema').value = usuario.rolSistema;
            if (usuario.idVoluntario) document.getElementById('voluntarioId').value = usuario.idVoluntario;

            document.getElementById('password').required = false;
            document.getElementById('confirmPassword').required = false;

            // Marcar los permisos que ya tiene el usuario
            document.querySelectorAll('.perm').forEach(ch => {
                ch.checked = usuario.permisos && usuario.permisos.includes(parseInt(ch.value));
            });
            actualizarContadores();
