...
public class ActividadController extends HttpServlet {
    ...
    /** Gson seguro con soporte para tipos de fecha de Java 8 */
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("listar".equals(action)) {
            ...
            request.getRequestDispatcher("/views/actividades/listar.html").forward(request, response);
        }
    }
    ...
    private void crearActividad(HttpServletRequest request, HttpServletResponse response, Usuario usuario)
        ...
        try {
            ...
            Actividad actividad = new Actividad(nombre, descripcion, fechaInicioDate, fechaFinDate, ubicacion, cupoMaximo);
            actividad.setIdUsuario(usuario.getIdUsuario());

            boolean resultado = actividadRepository.crearActividad(actividad);
    ...
    private void obtenerActividad(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            Actividad actividad = actividadRepository.obtenerActividadPorId(id);
            response.getWriter().write(GSON.toJson(actividad));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void actualizarActividad(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            actividad.setFechaInicio(request.getParameter("fechaInicio") != null && !request.getParameter("fechaInicio").isEmpty() ? LocalDate.parse(request.getParameter("fechaInicio")) : null);
            actividad.setFechaFin(request.getParameter("fechaFin") != null && !request.getParameter("fechaFin").isEmpty() ? LocalDate.parse(request.getParameter("fechaFin")) : null);
            ...
            boolean resultado = actividadRepository.actualizarActividad(actividad);
            if (resultado) {
    ...
    private void cambiarEstado(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            boolean resultado = actividadRepository.cambiarEstado(id, estado);
            if (resultado) {
    ...
    private void eliminarActividad(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            int id = Integer.parseInt(request.getParameter("id"));
            boolean resultado = actividadRepository.eliminarActividad(id);
            if (resultado) {
