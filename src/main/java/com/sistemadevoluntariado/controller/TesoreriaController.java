...
public class TesoreriaController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (tesoreriaRepository == null) tesoreriaRepository = new TesoreriaRepository();
        ...
        if (accion != null) {
            ...
            switch (accion) {
                case "listar":
                    resp.getWriter().write(gson.toJson(tesoreriaRepository.listar()));
                    return;

                case "obtener":
                    int id = Integer.parseInt(req.getParameter("id"));
                    MovimientoFinanciero m = tesoreriaRepository.obtenerPorId(id);
                ...
                case "filtrar":
                    ...
                    List<MovimientoFinanciero> filtrados = tesoreriaRepository.filtrar(tipo, categoria, fechaIni, fechaFin);
                    resp.getWriter().write(gson.toJson(filtrados));
                    return;

                case "resumenCategoria":
                    resp.getWriter().write(gson.toJson(tesoreriaRepository.resumenPorCategoria()));
                    return;

                case "resumenMensual":
                    resp.getWriter().write(gson.toJson(tesoreriaRepository.resumenMensual()));
                    return;

                case "eliminar":
                    int idEliminar = Integer.parseInt(req.getParameter("id"));
                    boolean eliminado = tesoreriaRepository.eliminar(idEliminar);
        ...
        req.setAttribute("movimientos", tesoreriaRepository.listar());
        req.getRequestDispatcher("/views/tesoreria/tesoreria.html")
                .forward(req, resp);
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ("actualizar".equals(accion)) {
            ...
            ok = tesoreriaRepository.actualizar(m);
        } else {
            ok = tesoreriaRepository.registrar(m);
        }
